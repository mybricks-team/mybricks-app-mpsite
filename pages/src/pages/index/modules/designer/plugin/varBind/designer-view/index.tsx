...
const DesignerView = (props: DesignerViewProps) => {
  ...
  const [tabChangeType, setTabChangeType] = useState(
    Math.random().toString(36).slice(-6)
  );
  ...
  const _generateUniqueTitle = (prefix) => {
    let titleNumber = 1;
    let uniqueTitle = false;
    let newTitle = "";
    while (!uniqueTitle) {
      newTitle = `${prefix}-同步项${titleNumber}`;
  ...
  const addVar = (type: itemTypes) => {
    ...
    const uniqueTitle = _generateUniqueTitle(titlePrefix);
                  ...
                  <div key={item.id} className={css.var}>
                    <div className={css.varTitleBox}>
                      <div className={css.varTitle}>
                        ...
                        <input
                          ...
                          type="text"
                          value={
                            tempInput[item.apiId] !== undefined
                              ? tempInput[item.apiId]
                              : item.title
                          }
                          placeholder="请输入变量名"
                    ...
                    <div className={css.varbox}>
                      <input
                        ...
                        onBlur={(e) =>
                          handleInputValueBlur(item, e.target.value)
                        }
                        value={
                          tempInputValue[item.apiId] !== undefined
                            ? tempInputValue[item.apiId]
                            : item.value
                        }
                        type="text"
